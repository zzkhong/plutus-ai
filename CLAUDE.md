...
A user moves `onboarding` → `pending_approval` → `approved`. `/setup` checks a pasted key with one live call; `classifyKeyFailure` tells a rejected key apart from a rate limit or an unavailable model or outage, so nobody is told to replace a working key, and the cause is logged with the key redacted. `completeSetup` auto-approves when `isAdmin` (the `ADMIN_CHAT_ID` bootstrap) and also when the user already had an encrypted key — a key *rotation* keeps approval and reuses the existing `webhook_api_key`.
...
The first middleware, `chatSequencer` ([src/bot/middleware/sequence.ts](src/bot/middleware/sequence.ts)), runs each chat's updates one at a time, in `update_id` order. The webhook acknowledges updates at once, so without it two messages sent seconds apart run concurrently — possibly on different instances — and "actually 5" could correct the expense before the one just sent. Each update registers in the `chat_updates` table and polls until the chat's earlier updates are done (at most 60s; a registration older than 3 minutes is treated as a crashed instance's and ignored). Under long polling it never waits. Then `authMiddleware` ([src/bot/middleware/auth.ts](src/bot/middleware/auth.ts)) resolves `ctx.chat.id` to a `users` row and attaches it as `ctx.user`. Unregistered and `pending_approval` chats get a canned reply unless the message is `/setup` or `/help`; an `onboarding` chat can also send plain text (the setup conversation), but no photos, voice notes, files, buttons or other commands, since it has no API key yet. Command handlers start with `if (!ctx.user) return;` and pass `ctx.user.id` down.
...
[src/expense/service.ts](src/expense/service.ts): `logExpense(userId, data)` resolves currency ([currency-resolver.ts](src/expense/currency-resolver.ts) — explicit > card mapping > a whole-word RM/MYR, US$/USD or S$/SGD marker in the merchant or note > SGD; a bare "$" is no marker, because a Singapore iPhone writes S$4.50 as "$4.50", and "rm" inside "Supermarket" is no ringgit sign), picks a category, converts to SGD cents via `toSGD` with the day's rates from `getExchangeRates()` (see [Exchange rates](#exchange-rates)), and inserts.
...
- `fireRecurringForToday(userId)` logs each active template's charge for every due day among the last three — today, plus two days a skipped cron run may have missed (a caught-up charge is dated the day it fell due, and only if the template's `updated_at` is on or before that day). It logs a charge once per template per day, keyed by `recurring_id` and the spent day, so it's safe to run any number of times: Vercel Cron can deliver twice, and the standalone process runs it at startup and at midnight. `isDueOn` puts a day the month lacks on its last day, so a charge on the 31st still lands in September.
