...
Current build state: foundation, the Telegram bot shell, the expense engine, the budget system, the portfolio tracker, the expense split calculator, the daily digest, and the iOS Shortcuts webhook (tasks 01–08) are implemented and tested. **All free-text intents are now wired to real actions** — see the intent-by-intent breakdown below. Voice messages remain a complete stub. Verify against `buildAssistantReply` in [src/bot/ai.ts](src/bot/ai.ts) before trusting a stale claim here — this file has been wrong about intent wiring and the portfolio tracker's status before.
...
There is no test-file globbing — `npm test` runs a fixed list of `*.test.ts` files (currently `src/bot/ai.test.ts`, `src/expense/expense.test.ts`, `src/budget/service.test.ts`, `src/budget/progress.test.ts`, `src/budget/alerts.test.ts`, `src/scheduler/recurring.test.ts`, `src/digest/digest.test.ts`, `src/webhook/webhook.test.ts`, `src/portfolio/service.test.ts`, `src/portfolio/price-fetcher/crypto.test.ts`, `src/portfolio/price-fetcher/stocks.test.ts`, `src/portfolio/price-fetcher/index.test.ts`, `src/portfolio/calculator.test.ts`, `src/portfolio/statement-parser.test.ts`, `src/bot/handlers/document.test.ts`, `src/split/calculator.test.ts`, `src/split/extraction.test.ts`, `src/split/assignment.test.ts`, `src/split/state.test.ts`, `src/bot/commands/split.test.ts`) via `npx tsx --test`. To run a single test file or filter by name:
...
**Voice messages are a complete stub**: [src/bot/handlers/voice.ts](src/bot/handlers/voice.ts)'s `handleVoiceMessage` does no transcription at all — it logs the file id and returns a canned "will be processed once the speech-to-text layer is connected" string, regardless of content.
