...
[src/portfolio/](src/portfolio/): `service.ts` (holdings CRUD, `userId`-scoped; `addHolding`/`removeHolding` only touch manual rows where `broker IS NULL`; `replaceHoldingsForBroker` replaces one broker's rows for one user in a single `batch`), `statement-parser.ts` (`parseStatement(userId, pdf)` — the user's provider reads an IBKR/Moomoo PDF into strict JSON, throws `StatementParseError`, no fallback), `price-fetcher/` (Yahoo Finance for US/MY/SG stocks via the hand-maintained `symbol-map.ts`, CoinGecko for crypto, in-memory TTL cache), `calculator.ts` (pure net worth/allocation math), and `advice.ts` (`generatePortfolioAdvice` — see digest). PDF uploads are handled end-to-end by [src/bot/handlers/document.ts](src/bot/handlers/document.ts) with no confirmation step. The extraction prompt is still unvalidated against real IBKR/Moomoo PDFs.
...
- [src/config/currencies.ts](src/config/currencies.ts): the `Currency` union, `DEFAULT_CARD_CURRENCY_MAP`, and hardcoded `EXCHANGE_RATES` (update manually if rates drift).
