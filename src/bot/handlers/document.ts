/**
 * Statement import, from any broker and any file the statement reader can
 * handle: a PDF, a screenshot sent as a file, or a CSV/text export. The
...
import { parseStatement, StatementFileKind, StatementParseError } from '../../portfolio/statement-parser';
...
export async function handleDocumentMessage(
  ...
  if (!kind) {
    return "I can read statements sent as a PDF, a screenshot or a CSV export, and that file isn't one of those. If it's a spreadsheet, export it as PDF or CSV first.";
  }
  ...
  try {
    ...
    if (error instanceof StatementParseError) {
      ...
      return `I couldn't read that statement: ${error.message}. Try sending it again, or another export of it such as a PDF.`;
    }
    throw error;
  }
