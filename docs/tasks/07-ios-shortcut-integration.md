...
**Status: implemented.** See `src/webhook/` (server, auth, route, types)
and `doc/ios-shortcut-setup.md` for the user-facing setup guide.

- [x] HTTP POST endpoint accepts transaction data from iOS Shortcut
- [x] Endpoint is secured (API key or shared secret in header) — `x-api-key`
      checked against `WEBHOOK_API_KEY`; the webhook server refuses to bind
      at all if `WEBHOOK_API_KEY` is unset (rest of the app still starts)
- [x] Payload includes: amount, merchant name, card name used
- [x] Card name is mapped to currency via configured mapping (existing
      `resolveCurrency`/`DEFAULT_CARD_CURRENCY_MAP`, unchanged)
- [x] Transaction is logged via the Expense Engine (`logExpense` with
      `source: 'apple_pay'`)
- [x] Confirmation message sent to user via Telegram:
      "Spent $4.50 at Ya Kun — Food"
- [x] Response time < 3 seconds from webhook to Telegram confirmation — no
      artificial delays; Telegram send happens synchronously in-request but
      failures there don't fail the HTTP response (transaction is already
      logged by that point)
- [x] If webhook fails, iOS Shortcut shows an error (user knows to
      log manually) — non-2xx status on any failure (401/400/500)
- [x] iOS Shortcut template documented for user to set up —
      `doc/ios-shortcut-setup.md`

### Implementation decisions

- **Deployment**: Cloudflare Tunnel **quick tunnel**
  (`cloudflared tunnel --url http://localhost:PORT`) — no Cloudflare
  account/domain needed, at the cost of the public URL changing on every
  tunnel restart. A named tunnel is a possible future upgrade if the
  churn becomes annoying.
- **Server**: a separate Hono app served via `@hono/node-server` on
  `config.PORT`, started from `src/index.ts` alongside the existing
  schedulers. Independent of the Telegram bot's long-polling loop.
- **Auth failure mode**: `WEBHOOK_API_KEY` is optional in the env schema
  (so existing setups/tests that don't touch the webhook are unaffected),
  but `startWebhookServer` won't bind the port at all if it's missing —
  logs an error and skips, same pattern as the Telegram bot being skipped
  when `TELEGRAM_BOT_TOKEN` is absent.
- **Amount parsing**: strips non-numeric characters from the `amount`
  string and treats a leading currency symbol/code (e.g. `"RM 45.00"`) as
  an explicit currency override via the existing
  `parseExplicitCurrency` helper, falling back to card/merchant-based
